<%@ Page Title="" Language="C#" MasterPageFile="~/EMS.Master" AutoEventWireup="true"
    CodeBehind="GeographicDashboardFacilities.aspx.cs" Inherits="Condesus.EMS.WebUI.Dashboard.GeographicDashboardFacilities" %>
...
//                _MaxLatitude = parseFloat(document.getElementById('inputPointProcess').value.split(';')[1].split('|')[0]);
//                _MinLatitude = parseFloat(document.getElementById('inputPointProcess').value.split(';')[1].split('|')[0]);

//                _MaxLongitude = parseFloat(document.getElementById('inputPointProcess').value.split(';')[1].split('|')[1]);
//                _MinLongitude = parseFloat(document.getElementById('inputPointProcess').value.split(';')[1].split('|')[1]);
//            }
//            
...
//            }
        ...
        function showFacebook() {
            var dock = $find('<%= RDockFacebook.ClientID%>');
            var isClosed = dock.get_closed();
            dock.set_closed(!isClosed);
            var dock1 = $find('<%= RDockTwitter.ClientID%>');
            var isClosed1 = dock.get_closed();
            dock1.set_closed(isClosed);

        }
        function showTwitter() {
            var dock = $find('<%= RDockTwitter.ClientID%>');
            var isClosed = dock.get_closed();
            dock.set_closed(!isClosed);
            var dock1 = $find('<%= RDockFacebook.ClientID%>');
            var isClosed1 = dock.get_closed();
            dock1.set_closed(isClosed);
        }
        ...
        function showPopUpSearch(eventArgs) {
            ...
            //            StopEvent(eventArgs);
            //Busca el popup
            var modalPopupBehavior = $find('programmaticModalPopupBehaviorSearch');
            //muestra el popup
    ...
    <input id="IdProcess" type="hidden" value="<% =IdProcess%>" />
    <input id="TwitterUser" type="hidden" value="<% =_TwitterUser%>" />
    <rad:RadDockLayout ID="RDockLayoutMainPage" runat="server" Skin="EMS">
        <table style="position: absolute; z-index: 99;">
            <tr>
                <td style="padding-left: 70px;">
                        <rad:RadDock ID="RDockTwitter" Title="Twitter" runat="server" Width="402px"  Height="250px" DockMode="Docked"
                            Top="20px" Left="70px" Text="Aca va el Plugin de Twitter" OnClientDragEnd="OnClientDragEnd"
                            Skin="EMS" Closed="true">
                            <ContentTemplate>
                                <asp:Panel ID="pnlTwitter" runat="server">

                                    <script type="text/javascript" src="http://widgets.twimg.com/j/2/widget.js"></script>
                                    <script type="text/javascript">
                                        var _twitterUser = document.getElementById('TwitterUser').value;
                                        new TWTR.Widget({
                                            version: 2,
                                            type: 'profile',
                                            rpp: 4,
                                        ...
                                        }).render().setUser(_twitterUser).start();
                                </script>

                                </asp:Panel>
                            </ContentTemplate>
                        </rad:RadDock>
                         <rad:RadDock ID="RDockFacebook" Title="Facebook" runat="server" Width="400px" DockMode="Docked"
                            Top="45px" Left="70px" Height="250px" Text="Aca va el Plugin de Facebook" OnClientDragEnd="OnClientDragEnd"
                            Skin="EMS" Closed="true">
                            <ContentTemplate>
                                <asp:Panel ID="pnlFb" runat="server">
                                    <div id="fb-root">
                                    </div>
                                    ...
                                    <div id="pnlFBComments" runat="server" class="fb-comments" data-href="http://www.facebook.com/?ref=tn_tnmn#!/pages/ghree/102361213185483"
                                        data-num-posts="3" data-width="381" data-height="200">
                                    </div>
                                </asp:Panel>
                            </ContentTemplate>
                        </rad:RadDock>
    ...
    <div id='FilterMap'>
        <asp:Button ID="btnTwitter" runat="server" Text="Twitter" OnClientClick="javascript:showTwitter(this);" CssClass="btnShowTwitter" />
        <asp:Button ID="btnFacebook" runat="server" Text="Facebook" OnClientClick="javascript:showFacebook(this);" CssClass="btnShowFacebook"/>
    ...
    <asp:Panel ID="pnlPopUpSearch" runat="server">
        ...
        <ajaxToolkit:ModalPopupExtender ID="mpeSearch" runat="server" TargetControlID="btnHidden"
            BehaviorID="programmaticModalPopupBehaviorSearch" PopupControlID="pnlConfirmSearch"
    ...
    <asp:Panel ID="pnlPopUpFilter" runat="server">
        ...
        <ajaxToolkit:ModalPopupExtender ID="mpeFilter" runat="server" TargetControlID="btnHiddenFilter"
            ...
            Enabled="True" />
        <div>
            <asp:Panel ID="pnlConfirmFilter" runat="server" Style="display: none" CssClass="pnlBgSearch">
                <asp:Label ID="lblTitleFilter" runat="server" Text="Filter" CssClass="lblTitleSearch"></asp:Label>
                <asp:Label ID="lblSubTitleFilter" runat="server" Text="Filter Accounting Activity"
                    CssClass="lblSubTitleSearch"></asp:Label>
                <br />
                <div style="position: absolute; top: 70px; left: 55px;">
                    <asp:PlaceHolder ID="phActivities" runat="server"></asp:PlaceHolder>
                </div>
                <asp:Button ID="btnFilter" runat="server" Text="Apply" OnClientClick="FilterActivity();"
                    CausesValidation="false" CssClass="btnSearchApply" />
                <asp:Button ID="btnCloseFilter" runat="server" Text="x" CssClass="btnSearchClose" />
            </asp:Panel>
        </div>
    </asp:Panel>
